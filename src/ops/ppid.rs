...
impl VM {
    ...
    pub fn pre_process(&mut self, reg: &Register) -> (i16, usize) {
        let process_cost = self.process_arg(reg, false);
    ...
    #[inline(always)]
    #[must_use]
    pub fn post_process(&mut self, reg: &Register) -> usize {
        self.process_arg(reg, true)
    }

    #[must_use]
    fn process_arg(&mut self, reg: &Register, is_post: bool) -> usize {
        if reg.is_calc && reg.is_post == is_post {
            if reg.is_inc {
                match reg.size {
                    1 => {
                        let (value, _) = self.read_byte_reg_value(reg);
                        return self.write_byte_reg_value(reg, value.wrapping_add(1));
                    }
                    2 => {
                        let (value, _) = self.read_word_reg_value(reg);
                        return self.write_word_reg_value(reg, value.wrapping_add(2));
                    }
                ...
                match reg.size {
                    1 => {
                        let (value, _) = self.read_byte_reg_value(reg);
                        return self.write_byte_reg_value(reg, value.wrapping_sub(1));
                    }
                    2 => {
                        let (value, _) = self.read_word_reg_value(reg);
                        return self.write_word_reg_value(reg, value.wrapping_sub(2));
                    }
                    _ => self.fail(format!("Invalid register size: {}", reg.addr)),
                }
            }
